    ...
    return class ShiningService extends _classSuper {
        ...
        constructor(ctx, _config) {
            ...
            ctx.inject(['settings'], (settingsCtx) => {
                ...
                const readSettings = () => settingsCtx.settings.get(settingsNamespace(SETTINGS_NAMESPACE)) ?? DEFAULT_SHINING_SETTINGS;
                void (async () => {
        ...
        async chat(request) {
            try {
                ...
                if (!response.ok)
                    return failure('chat-error', `upstream ${response.status}`);
        ...
        async qqSend(request) {
            try {
                const svc = this.ctx.shiningQq;
                if (!svc)
                    return failure('qq-error', 'qq service not enabled');
