...
import { listMemoryNotes, saveMemoryNote } from './storage.ts'

interface MnemeItem { title?: string; content?: string; name?: string }
 ...
 */
export async function readMemoryContext(query: string): Promise<string> {
  const chunks: string[] = []
  try {
    const res = await fetch(`/api/dsh-mneme/search?q=${encodeURIComponent(query)}&limit=5`)
    if (res.ok) {
      const data = (await res.json()) as { items?: MnemeItem[] }
...
export function saveMemory(content: string): Promise<void> {
